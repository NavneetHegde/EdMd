...
- **JS → C# (`OnWebMessageReceived`):** the page sends `{type}` messages —
  ...
  (a `dark` bool, to match the native title bar — see Gotchas), `exportHtml`/`exportPdf` (both
  carry the active tab's `name` + a **standalone HTML document string JS built** from the
  editor's rendered HTML — see "Export" below), and `readyToClose` (the
